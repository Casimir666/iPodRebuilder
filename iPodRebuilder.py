...
def get_relpath(path, basepath):
    commonprefix = os.sep.join(os.path.commonprefix([path, basepath]))
    return os.path.relpath(path, commonprefix)

def is_path_prefix(prefix, path):
    return prefix == os.sep.join(os.path.commonprefix([prefix, path]))
...
class Text2Speech(object):
    ...
    def guess_lang(unicodetext):
        lang = 'en-GB'
        if re.search(u"[?-??-?]", unicodetext) is not None:
            lang = 'ru-RU'
        return lang
